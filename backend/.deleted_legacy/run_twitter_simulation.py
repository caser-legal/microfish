...
This script reads configuration file parameters and executes the simulation, fully automating the process.

Features:
- Does not close the environment immediately after the simulation completes; enters command wait mode
...
class IPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        """
        Process a batch interview command.
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Uses the project root directory .env file configuration (highest priority):
        - LLM_API_KEY: API key
        - LLM_BASE_URL: API base URL
        - LLM_MODEL_NAME: model name
        """
        # First read configuration from .env
        llm_api_key = os.environ.get("LLM_API_KEY", "")
        llm_base_url = os.environ.get("LLM_BASE_URL", "")
        llm_model = os.environ.get("LLM_MODEL_NAME", "")

        # If .env does not have it, fall back to config
        if not llm_model:
            llm_model = self.config.get("llm_model", "gpt-4o-mini")

        # Set up the environment variables required by camel-ai
        if llm_api_key:
    ...
    def _get_active_agents_for_round(
        ...
        """
        Determine the agents active in this round based on time and configuration.
        ...
        # Calculate activity probability from each agent's configuration
        candidates = []
        for cfg in agent_configs:
            ...
            # Check whether within active hours
    ...
    async def run(self, max_rounds: int = None):
        ...
        print(f"  - Simulated duration: {total_hours} hours")
        print(f"  - Time per round: {minutes_per_round} minutes")
        print(f"  - Round count: {total_rounds}")
        if max_rounds:
            print(f"  - Maximum round limit: {max_rounds}")
        ...
        if os.path.exists(db_path):
            os.remove(db_path)
            print(f"Deleted old data: {db_path}")
        ...
        if initial_posts:
            print(f"Executing initial event ({len(initial_posts)} initial posts)...")
            initial_actions = {}
            ...
            if initial_actions:
                ...
                print(f"  Posted {len(initial_actions)} initial posts")
...
def setup_signal_handlers():
    ...
    The program gets a chance to clean up resources normally (close database, environment, etc.).
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Simulation entry program exited")
