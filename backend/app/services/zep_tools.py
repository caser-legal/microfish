...
Provides tools such as graph search, node reading, and edge queries for the Report Agent to use
...
2. PanoramaSearch (breadth search) - gets the full picture, including historical content
...
class SearchResult:
    ...
    def to_text(self) -> str:
        """Convert to a text format for the LLM to understand."""
        text_parts = [f"Search query: {self.query}", f"Found {self.total_count} relevant facts"]
...
class InsightForgeResult:
    ...
    Contains the results of multiple sub-query retrievals for combined analysis
    """
    query: str
    ...
    def to_text(self) -> str:
        """Convert to a detailed text format for the LLM to understand."""
        text_parts = [
        ...
        if self.semantic_facts:
            text_parts.append("\n### Key Facts (use this original content in the report)")
...
class PanoramaResult:
    ...
    Contains relevant information, including historical content
    """
    query: str
    ...
    # Historical/expired facts (historical records)
    ...
    def to_text(self) -> str:
        """Convert to a text format (complete version, not truncated)."""
        text_parts = [
            "## Breadth Search Results (full-scenario view of the graph)",
            f"Query: {self.query}",
            "\n### Statistics",
            f"- Nodes: {self.total_nodes}",
            f"- Edges: {self.total_edges}",
            f"- Currently valid facts: {self.active_count}",
            f"- Historical/past facts: {self.historical_count}"
        ]
        
        # Currently valid facts (complete output, not truncated)
        if self.active_facts:
            text_parts.append("\n### Current Valid Facts (simulation result content)")
            for i, fact in enumerate(self.active_facts, 1):
                text_parts.append(f"{i}. \"{fact}\"")
        
        # Historical/past facts (complete output, not truncated)
        if self.historical_facts:
            text_parts.append("\n### Historical/Past Facts (evolution process records)")
...
class AgentInterview:
    ...
    def to_text(self) -> str:
        ...
        # Show the complete agent_bio, not truncated
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out content containing the question-number prefix
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate overly long content (at sentence boundaries, not hard truncation)
...
class InterviewResult:
    ...
    Contains responses from multiple simulated agent interviews
    """
    interview_topic: str  # Interview main question
    interview_questions: List[str]  # Interview question list
    
    # Selected interview agents
    ...
    def to_text(self) -> str:
        """Convert to a detailed text format for the LLM to understand and for report use."""
        text_parts = [
            "## In-Depth Interview Report",
            f"**Interview main question:** {self.interview_topic}",
...
class ZepToolsService:
    ...
    2. panorama_search - breadth search (gets the full picture, including historical content)
    3. quick_search - simple search (fast retrieval)
    4. interview_agents - in-depth interview (interviews simulation agents, obtains multiple perspectives)
    ...
    def _call_with_retry(self, func, operation_name: str, max_retries: int = None):
        """Call the API with a built-in retry mechanism."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"Zep {operation_name} still failed after {max_retries} retries: {str(e)}")
    ...
    def _local_search(
        ...
        """
        Local keyword matching search (fallback for when the Zep Search API is unavailable)
        
        Fetches edges/nodes, then performs local keyword matching
        ...
        def match_score(text: str) -> int:
            """Calculate the match score between the text and the query."""
            if not text:
                return 0
            text_lower = text.lower()
            # Exact match with the full query
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        """
        Get the graph's nodes (paginated retrieval)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get the graph's edges (paginated retrieval, including temporal info)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches the graph's edges, then filters out the edges related to the specified node
        ...
        try:
            # Fetch the graph's edges, then filter
            all_edges = self.get_all_edges(graph_id)
            
            result = []
            for edge in all_edges:
                # Check whether the edge is related to the node (as source or target)
    ...
    def get_entity_summary(
        ...
        Search for information relevant to the entity and generate a summary
        ...
        # Try to find the entity among the existing nodes
        all_nodes = self.get_all_nodes(graph_id)
        entity_node = None
        for node in all_nodes:
    ...
    def get_simulation_context(
        ...
        Combines search with information relevant to the simulation requirement
        
        Args:
            graph_id: Graph ID
            simulation_requirement: Simulation requirement description
            limit: Limit on the number of items per type
        ...
        # Entities with a concrete type (not plain entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function; automatically decomposes the question and retrieves across multiple dimensions:
        1. Uses the LLM to decompose the question into multiple sub-questions
        2. Performs a semantic search for each sub-question
        3. Extracts relevant entities and fetches their detailed information
        4. Builds relationship chains
        5. Combines all results to generate a deep insight
        
        Args:
            graph_id: Graph ID
            query: User question
            simulation_requirement: Simulation requirement description
            report_context: Report context (optional; used for finer sub-query generation)
        ...
        # Step 2: Perform a semantic search for each sub-question
        all_facts = []
        all_edges = []
        seen_facts = set()
        ...
        # Step 3: Extract relevant entity UUIDs from edges, retrieving only those entities (not all nodes)
        entity_uuids = set()
        ...
        # Get all relevant entity info (no count limit, complete output)
        entity_insights = []
        node_map = {}  # Used later for relationship chain building
        
        for uuid in list(entity_uuids):  # Process all entities, not truncated
            if not uuid:
                continue
            try:
                # Fetch each relevant node's information individually
                node = self.get_node_detail(uuid)
                if node:
                    node_map[uuid] = node
                    entity_type = next((l for l in node.labels if l not in ["Entity", "Node"]), "Entity")
                    
                    # Get the facts relevant to this entity (not truncated)
                    related_facts = [
        ...
        # Step 4: Build the relationship chains (no count limit)
        relationship_chains = []
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are a specialist question-analysis expert. Your task is to decompose a complex question into multiple sub-questions that can be independently observed in the simulation world.
...
Return a JSON-format list of sub-queries."""
        ...
        try:
            ...
            # Ensure it remains a list of strings
            return [str(sq) for sq in sub_queries[:max_queries]]
            
        except Exception as e:
            logger.warning(f"Sub-query generation failed: {str(e)}; using default sub-queries")
            # Fallback: return variations based on the original question
            return [
                query,
    ...
    def panorama_search(
        ...
        Get the full-scenario view of the graph, including relevant content and historical/past information:
        1. Fetch relevant nodes
        2. Fetch edges (including historical/expired)
        3. Categorize by type to logically separate currently valid and historical information
        
        This tool is used when you need to understand the full picture of an event and its evolution.
        ...
        # Fetch edges (including temporal info)
        ...
        for edge in all_edges:
            ...
            # Determine whether it is historical/expired
            is_historical = edge.is_expired or edge.is_invalid
            
            if is_historical:
                # Historical/past facts; add a time marker
        ...
        # Relevance-sort based on the query
        query_lower = query.lower()
        keywords = [w.strip() for w in query_lower.replace(',', ' ').replace('，', ' ').split() if len(w.strip()) > 1]
    ...
    def quick_search(
        ...
        1. Directly call the Zep semantic search
        2. Return the most relevant results
        3. For simple, direct retrieval needs
    ...
    def interview_agents(
        ...
        Invokes the real OASIS interview API to interview agents currently running in the simulation:
        1. Automatically reads the persona files to get all simulation agents
        2. Uses the LLM to analyze the interview requirement and intelligently select the most relevant agents
        3. Uses the LLM to generate interview questions
        4. Calls the /api/simulation/interview/batch endpoint to perform a real interview (interviewing both platforms simultaneously)
        5. Combines all interview results and generates an interview report
        
        [Important] This function requires the simulation environment to be running (the OASIS environment must not be closed)
        
        [Use cases]
        - When you need to understand the event from different role perspectives
        - When you need to gather diverse opinions and viewpoints
        - When you need the simulation agents' real answers (not LLM simulation)
        
        Args:
            simulation_id: Simulation ID (used to locate the persona files and call the interview API)
            interview_requirement: Interview requirement description (free-form, e.g. "understand students' views on the event")
        ...
        # Step 2: Use the LLM to select agents to interview (returns a list of agent_ids)
        ...
        # Step 3: Generate interview questions (if none provided)
        ...
        # Combine the questions into a single interview prompt
        combined_prompt = "\n".join([f"{i+1}. {q}" for i, q in enumerate(result.interview_questions)])
        
        # Add an optimized prefix to constrain the agent reply format
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Please combine your persona, relevant memories, and actions, "
            "and answer the following questions in plain text.\n"
            "Reply requirements:\n"
            "1. Answer in your own words; do not invoke any tools\n"
            "2. Do not return JSON or tool-invocation formats\n"
            "3. Do not use Markdown headings (e.g. #, ##, ###)\n"
            "4. Answer one question at a time; start each answer with 'Question X:' (X is the question number)\n"
            "5. Separate each answer with a blank line\n"
            "6. Answers must have substance; answer each question in 2-3 sentences\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"
        
        # Step 4: Call the real interview API (no platform specified; defaults to interviewing both platforms simultaneously)
        try:
            # Build the batch interview list (no platform; interview both platforms)
            interviews_request = []
            ...
            # Call the SimulationRunner batch interview method (no platform; interview both platforms)
            api_result = SimulationRunner.interview_agents_batch(
                simulation_id=simulation_id,
                interviews=interviews_request,
                platform=None,  # No platform; interview both platforms
                timeout=180.0   # Both platforms require more time
            )
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Clean up possible JSON tool-invocation wrappers
                twitter_response = self._clean_tool_call_response(twitter_response)
                reddit_response = self._clean_tool_call_response(reddit_response)

                # Annotate output with both-platform markers
                ...
                # Clean up the response text: remove markers, Markdown, etc.
                ...
                # Strategy 2 (fallback): regex-match the long text inside the "" quotes
                if not key_quotes:
                ...
                interview = AgentInterview(
                    ...
                    agent_bio=agent_bio[:1000],  # Truncate overly long bios
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may be closed; please keep the OASIS environment running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean up JSON tool-call wrappers in agent replies and extract the actual content."""
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        # Prefer reading the Reddit JSON format first
        ...
        # Try reading the Twitter CSV format
        ...
        if os.path.exists(twitter_profile_path):
            try:
                with open(twitter_profile_path, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        # Convert the CSV format to a unified format
                        profiles.append({
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: List of complete information for the selected agents
                - selected_indices: List of selected agent indices (for API calls)
        ...
        system_prompt = """You are a specialist interview expert. Your task is to select the most suitable interview targets from the simulation agent list according to the interview requirement.

Selection criteria:
1. The agent's persona is relevant to the interview topic
2. The agent may hold distinctive or valuable viewpoints
3. Select agents with diverse perspectives (e.g. supportive, opposed, neutral, expert, etc.)
...
{simulation_requirement if simulation_requirement else "none"}

Optional agent list ({len(agent_summaries)}):
{json.dumps(agent_summaries, ensure_ascii=False, indent=2)}

Please select at most {max_agents} agents most suitable for the interview, and explain your selection logic."""
        ...
        try:
            ...
            # Get the complete information of the selected agents
            selected_agents = []
            valid_indices = []
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a specialist interviewer. Based on the interview requirement, generate 3-5 in-depth interview questions.

Question requirements:
1. Open-ended questions that elicit detailed answers
2. Different roles may give different answers
3. Cover multiple dimensions: facts, opinions, feelings, etc.
4. Natural, like a real interview
5. Keep each question within 50 words, concise and clear
6. Ask questions directly; do not include background explanations or prefixes

Return JSON format: {"questions": ["question 1", "question 2", ...]}"""

        user_prompt = f"""Interview requirement: {interview_requirement}

Simulation background: {simulation_requirement if simulation_requirement else "none"}

Interview targets' roles: {', '.join(agent_roles)}
...
Please generate 3-5 interview questions."""
        ...
        try:
            ...
            return [
                ...
                "What impact does this matter have on the groups involved?",
                "How do you think this issue should be resolved or addressed?"
            ]
    ...
    def _generate_interview_summary(
        ...
        # Collect the interview content
        interview_texts = []
        for interview in interviews:
            interview_texts.append(f"[{interview.agent_name} ({interview.agent_role})]\n{interview.response[:500]}")
        
        system_prompt = """You are a specialist journalist. Based on the interviewees' answers, generate an interview summary.

Summary requirements:
1. Summarize each person's main viewpoints
2. Highlight consensus and differences among viewpoints
3. Highlight valuable insights
4. Stay objective; do not take any side
...
- Use quotation marks "" to quote the interviewees' original words
