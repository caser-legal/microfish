"""Zep Graph Pagination Tool.

Zep's node/edge list interface uses UUID cursor pagination.
This module encapsulates automatic pagination logic (with single-page retry),
transparently returning the complete list to the caller.
"""
...
def fetch_all_nodes(
    ...
    """Paginate through graph nodes, returns at most max_items entries (default 2000). Each page request has built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginate through all graph edges, returns complete list. Each page request has built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
