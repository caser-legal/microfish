...
Uses project context mechanism to persist state on server side
"""
...
def generate_ontology():
    ...
    try:
        ...
        logger.info(f"Text extraction completed, {len(all_text)} characters")
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "Project has not generated ontology, please call /ontology/generate first"
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is being built, please do not submit repeatedly. To force rebuild, add force: true",
                "task_id": project.graph_build_task_id
            }), 400
        
        # If forced rebuild, reset status
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'MicroFish Graph')
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "Graph build task started, please query progress via /task/{task_id}"
            }
        })
