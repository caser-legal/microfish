...
Step2: Zep entity read and filter, OASIS simulation prepare and run (fully automated)
"""
...
# Adding this prefix prevents Agent from calling tools, reply directly with text
INTERVIEW_PROMPT_PREFIX = "Combine your persona, all past memories and actions, reply to me directly with text without calling any tools:"


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview question, add prefix to prevent Agent from calling tools
...
def get_graph_entities(graph_id: str):
    ...
    Only return nodes matching predefined entity types (Labels is not just Entity)
...
def create_simulation():
    ...
    Note: max_rounds etc. parameters are intelligently generated by LLM, no need to set manually
    
    Request (JSON):
        {
            "project_id": "proj_xxxx",      // Required
            "graph_id": "mirofish_xxxx",    // Optional, if not provided will be fetched from project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: the run scripts (run_*.py) remain in the backend/scripts/ directory and are no longer copied to the simulation directory
    ...
    # Required file list (scripts not included, scripts located in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # - preparing: If config_generated=True indicates completion
        # - running: Running, indicates preparation was completed long ago
        # - completed: Completed, indicates preparation was completed long ago
        # - stopped: Stopped, indicates preparation was completed long ago
        ...
        if status in prepared_statuses and config_generated:
            ...
            logger.info(f"Simulation {simulation_id} detection result: Preparation complete (status={status}, config_generated={config_generated})")
            return True, {
                "status": status,
            ...
            logger.warning(f"Simulation {simulation_id} detection result: Preparation not complete (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    ...
    This is a time-consuming operation. The endpoint immediately returns a task_id,
    which can be queried via GET /api/simulation/prepare/status for progress.
    
    Features:
    - Automatically detect completed preparations to avoid duplicate generation
    - if preparation complete, directly return existing results
    - Support forced regeneration (force_regenerate=true)
    
    Steps:
    1. Check whether there is completed preparation work
    2. Read and filter entities from the Zep graph
    3. Generate an OASIS Agent Profile for each entity (with retry mechanism)
    4. Use the LLM to intelligently generate the simulation configuration (with retry mechanism)
    ...
    Request (JSON):
        {
            ...
            "use_llm_for_profiles": true,                 // Optional, whether to use the LLM to generate profiles
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|There is completed preparation",
    ...
    try:
        ...
        # Check if preparation is complete (to avoid duplicate generation)
        if not force_regenerate:
            logger.debug(f"Checking whether simulation {simulation_id} preparation is complete...")
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "There is completed preparation, no need for duplicate generation",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            else:
                logger.info(f"Simulation {simulation_id} preparation not complete, will start preparation task")
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": "Project lacks simulation requirement description (simulation_requirement)"
            }), 400
        ...
        force_domain = data.get('domain') or None
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Don't get edge info, speed up
            )
            # Save entity count to status (for frontend to immediately get)
...
def get_prepare_status():
    ...
    2. Check whether there is completed preparation work via simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // Whether there is completed preparation
    ...
    try:
        ...
        # if simulation_id provided, first check whether preparation complete
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "There is completed preparation",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task:
            # Task does not exist, but if there is a simulation_id, check whether preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverse reports directory, find report with matching simulation_id,
    If multiple exist, return the latest one (sorted by created_at)
            ...
            "data": [
                {
                    "simulation_id": "sim_xxxx",
                    "project_id": "proj_xxxx",
                    "project_name": "Alien Arrival Global Response",
                    "simulation_requirement": "How would global governments, media, and citizens react if aliens suddenly appeared on Earth?",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, use recommended rounds if not available
            ...
            # Get the associated project's file list (max 3)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get the simulation's Agent Profile in real time (for viewing progress during the generation process)
    
    Difference from /profiles endpoint:
    - Read file directly, not through SimulationManager
    - Suitable for real-time viewing during generation
    - Return additional metadata (such as file modification time, whether generating, etc.)
    ...
    try:
        ...
        # Check whether generating (via state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Return additional metadata (such as file modification time, whether generating, etc.)
    - Can return partial info even if config is not fully generated
    ...
    try:
        ...
        logger.error(f"Failed to get configuration in real time: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        ...
        - generation_reasoning: Explanation of the LLM configuration reasoning
    """
    try:
        manager = SimulationManager()
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script file (common scripts, located in backend/scripts/)
...
def generate_profiles():
    ...
    try:
        ...
        if filtered.filtered_count == 0:
            return jsonify({
                "success": False,
                "error": "No entities matching conditions found"
            }), 400
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, will first stop and clean up run logs
        ...
        - Suitable for scenarios requiring re-running the simulation

    About enable_graph_memory_update:
        - When enabled, all of the simulation's Agent activity (posting, commenting, liking, etc.) will be updated to the Zep graph in real time
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "force_restarted": true               // Whether it's a forced restart
            }
        }
    """
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation work completed, check if there is a running process
                ...
                if force:
                    logger.info(f"Force mode: clean up simulation logs {simulation_id}")
                ...
                logger.info(f"Simulation {simulation_id} preparation work completed, reset status to ready (original status: {state.status.value})")
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling graph memory update requires valid graph_id, please ensure project has built graph"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for frontend to display real-time dynamics
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "twitter_actions": [...],  # All actions from Twitter platform
                "reddit_actions": [...]    # All actions from Reddit platform
            }
        }
    """
    try:
        ...
        # Generic tracks are the primary result. These fields preserve the old
        # twitter/reddit response shape for existing clients.
        track_actions = {}
        for track_name in (run_state.tracks or {}):
            track_actions[track_name] = [
                a.to_dict() for a in SimulationRunner.get_all_actions(
                    simulation_id=simulation_id, platform=track_name
                )
            ]
        
        # Get current round's actions (recent_actions only shows latest round)
        ...
        result["track_actions"] = track_actions
        result["rounds_count"] = len(run_state.rounds)
        # recent_actions only shows current latest round content from both platforms
...
def get_simulation_actions(simulation_id: str):
    """
    Get the Agent action history in the simulation
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used for frontend to display progress bar and timeline view
...
def get_agent_stats(simulation_id: str):
    ...
    Used for the frontend to display Agent activity level ranking, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in running state (entered wait command mode after completing simulation loop)

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // Required, simulation ID
            "agent_id": 0,                     // Required, Agent ID
            "prompt": "What is your opinion on this matter?",  // Required, interview question
    ...
    Returns (no platform specified, dual-platform mode):
        {
            ...
            "data": {
                "agent_id": 0,
                "prompt": "What is your opinion on this matter?",
                "result": {
                    "agent_id": 0,
                    "prompt": "...",
                    "platforms": {
    ...
    Returns (when a platform is specified):
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation is completed and entered wait command mode."
            }), 400
        
        # Optimize the prompt; add a prefix to prevent the Agent from calling tools
        ...
        return jsonify({
            "success": False,
            "error": f"Waiting for interview response timed out: {str(e)}"
        }), 504
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                {
                    "agent_id": 0,
                    "prompt": "What is your A opinion on?",
                    "platform": "twitter"      // Optional, specify this Agent's interview platform
                },
                {
                    "agent_id": 1,
                    "prompt": "What is your B opinion on?"  // If no platform is specified, the default is used
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by each item's platform)
                                               // If not specified: in dual-platform simulation, each Agent is interviewed on both platforms at the same time
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Interview list item {i+1}'s platform can only be 'twitter' or 'reddit'"
                }), 400
        ...
        # Optimize each interview item's prompt; add a prefix to prevent the Agent from calling tools
...
def get_env_status():
    ...
    Check whether the simulation environment is alive (can receive interview commands)
...
def close_simulation_env():
    ...
    Send close environment command to simulation, making it gracefully exit wait command mode.
    
    Note: This is different from /stop endpoint, /stop will forcibly terminate process,
    while this endpoint lets simulation gracefully close environment and exit.
