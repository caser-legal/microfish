<template>
  ...
  >
    ...
    <!-- Cards container (only shown when there are projects) -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card description (full simulation requirement displayed) -->
      ...
      <Transition name="modal">
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Playback not available hint -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be launched during runtime, historical playback not supported</span>
            </div>
          </div>
        </div>
      </Transition>
...
// Card layout configuration - adjusted for wider ratio
...
const containerStyle = computed(() => {
  ...
  // Calculate actual needed height: rows * card height + (rows-1) * gap + small bottom spacing
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Expand downward, increase spacing with title
    ...
    const x = offset * 35
    // Adjust starting position, closer to title but maintain appropriate spacing
...
// Format date (only show date part)
...
// Generate title from simulation requirement (take first 20 characters)
...
// Format simulation_id display (truncate first 6 characters)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (record latest target state regardless of animation status)
        pendingState = shouldExpand
        
        // Clear previous debounce timer (new scroll intent will override old)
        ...
        // If target state same as current state, no need to process
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounce delay for state switching to prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Release lock after animation completes, check if there are pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // After animation ends, check if there are new pending states
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay a short time before executing to avoid too fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin, viewport bottom contracts upward, need more scrolling to trigger expand
...
// Listen to route changes, reload data when returning to home page
...
onMounted(async () => {
  ...
  // Wait for DOM to render then initialize observer
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
.grid-pattern {
  ...
  /* Position from top-left, expand only at bottom when height changes, doesn't affect existing grid position */
...
/* Low saturation color scheme - Morandi colors */
