    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Combines context, automatically calls tools to extract entities and relationships from the knowledge graph, initializes simulation individuals, and gives them unique behaviors and memories based on real-world seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Real-world Seed Related Topics</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow, recommendation algorithms, each agent's active periods, posting frequency, event triggers, and other parameters based on simulation requirements and real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generates initial activation events and hot topics based on narrative direction, guiding the initial state of the simulation world
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation Rounds Configuration - Only shown after configuration is generated and rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MicroFish automatically plans and deduces reality <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round represents reality <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes passing</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If agent scale is 100: Estimated time about {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        If agent scale is 100: Estimated time {{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If running for first time, strongly recommend switching to 'Custom Mode' to reduce simulation rounds, for quick preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            ...
            <div class="info-item">
              <span class="info-label">Event-External MBTI</span>
          ...
          <div class="modal-section">
            <span class="section-label">Character Profile</span>
...
// State
const phase = ref(0) // 0: initialize, 1: generatepersonset, 2: generateconfig, 3: complete
...
const useCustomRounds = ref(false) // Default to using auto-configured rounds
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generating Agent profiles' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Enter config generation stage, start polling config
    if (!configTimer) {
...
// Calculate auto-generated rounds from config (do not use hardcoded default)
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure maximum rounds is not less than 40 (recommended value), to avoid slider range issues
...
// Calculate total related topics count for all profiles
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // User chooses to keep auto-generated rounds, do not pass max_rounds parameter
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Immediately set expected Agent total (from prepare interface return value)
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(`fromZepgraphreadto ${res.data.expected_entities_count} entities`)
      ...
      startPolling()
      // Start getting Profiles in realtime
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agent profiles ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || 'Unknown profession'})`)
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog('Generating Agent profiles config...')
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time between config: Per round${tc.minutes_per_round}minutes, total${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}rounds`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup completed, can start simulation')
...
const loadPreparedData = async () => {
  ...
  // Get config (using realtime interface)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Config generating, start polling and waiting...')
        startConfigPolling()
      }
    }
  } catch (err) {
