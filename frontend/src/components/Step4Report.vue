          ...
          <div class="report-header-block">
            <div class="report-meta">
              ...
              <div class="report-controls">
                <button v-if="isGenerating" class="report-stop-btn" @click="stopReport">Stop</button>
                <button v-else class="report-start-btn" @click="startReport">Start / Resume</button>
              </div>
            </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation complete, but entire section may not be finished) -->
...
// Explicit report controls. Completed sections are persisted by the backend;
// Start / Resume never replays them as a new report unless the user explicitly
// requests a fresh generation from the parent workflow.
const startReport = async () => {
  ...
  try {
    ...
    const response = await fetch(`${apiBase}/report/generate`, {
      ...
      body: JSON.stringify({ simulation_id: props.simulationId, force_regenerate: false })
    })
    const data = await response.json()
    if (!response.ok || data.success === false) throw new Error(data.error || 'Could not start report')
    addLog('Report generation started or resumed')
  } catch (error) {
    isGenerating.value = false
    addLog(`Report start failed: ${error.message}`)
  }
}
...
const stopReport = async () => {
  ...
  try {
    const response = await fetch(`${apiBase}/report/${props.reportId}/stop`, { method: 'POST' })
    const data = await response.json()
    if (!response.ok || data.success === false) throw new Error(data.error || 'Could not stop report')
    isGenerating.value = false
    addLog('Report generation stop requested; completed sections were preserved')
  } catch (error) {
    addLog(`Report stop failed: ${error.message}`)
  }
}
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep button in same location
...
const toolConfig = {
  ...
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Lightning icon - represents quick search
  },
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/Analyze question:\s*(.+?)(?:\n|$)/)
    ...
    const entityMatch = text.match(/plusentity:\s*(\d+)/)
    ...
    // Extract core entities - extract completely, including summary and related fact count
    ...
    // Extract relationship chains - extract completely, no limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/whenbeforehaveeffectmattersolid:\s*(\d+)/)
    const histMatch = text.match(/historical\/pastperiodmattersolid:\s*(\d+)/)
    ...
    // Extract current active facts - extract completely, no limit
    const activeSection = text.match(/### 【whenbeforehaveeffectmattersolid】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    const histSection = text.match(/### 【historical\/pastperiodmattersolid】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
    ...
    const entitySection = text.match(/### 【plusentity】\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### Reasons for interviewee selection\n([\s\S]*?)(?=\n---\n|\n### visitsolidrecord)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse each persons selection reason
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **Government_Official_001 (index=1)**: As a government official responding to alien contact...
        ...
        } else if (currentName && line.trim() && !line.match(/^Not selected|^up|^Final selection/)) {
          // Continuation lines for reason (exclude ending summary paragraph)
          currentReason.push(line.trim())
        }
      }
      
      // Save last persons reason
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    const interviewBlocks = text.split(/#### visit #\d+:/).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (nameRoleMatch) {
        ...
        // Set this persons selection reason
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special handling if first question has "1." prefix
      ...
      // Extract answer - separate Twitter and Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*key|$)/)
      if (answerMatch) {
        ...
        // Platform fallback logic (compatible with old format: single platform marker)
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Latest key facts associated with chronological memory'),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.relations.length} chains`)
          ]),
          ...
          }, expandedRelations.value ? `Collapse ▲` : `Expand all ${props.result.relations.length} chains ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" format (Chinese format, new backend format)
      // 2. "1. " or "\n1. " (number+dot, old format compatibility)
      let matches = []
      let match

      // Prefer trying "Question X:" format
      const cnPattern = /(?:^|[\r\n]+)question(\d+)[：:]\s*/g
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return full answer for first question, empty for rest
    ...
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholder) and different content
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch button (only show when there are real dual-platform answers)
                  ...
                  // Expand/Collapse Button (do not show for placeholder text)
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.edges.length} relationships`)
          ]),
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 heading (## xxx), as section title is displayed in outer layer
  ...
  // Process line breaks - empty lines become paragraph separators, single breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean <p><br> following block elements (caused by extra empty lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph start tags following block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-continuous ordered lists: maintain incremental numbering when single <ol> items are separated by paragraph content
        ...
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Auto-expand newly generated sections
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic handled uniformly in nextTick after loop completes
          }
...
const extractFinalContent = (response) => {
  ...
  const chineseFinalMatch = response.match(/FinalAnswer[:：]\s*\n*([\s\S]*)$/i)
  ...
  // If content is long and contains markdown format, try removing thought process before returning
...
.report-controls { margin-left: auto; display: flex; gap: 8px; }
.report-stop-btn, .report-start-btn { padding: 6px 12px; border: 1px solid #bbb; background: #fff; cursor: pointer; font: inherit; }
.report-stop-btn { color: #b42318; border-color: #b42318; }
.report-start-btn { color: #1261a0; border-color: #1261a0; }
