        ...
        <TransitionGroup name="timeline-item">
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                <!-- Domain-agnostic summary for every action. Legacy social-specific
                     cards below remain as richer rendering when their fields exist. -->
                <div class="generic-action-summary">
                  <span v-if="!action.success" class="action-failure">REJECTED: {{ action.failure_reason || action.result || 'world constraint' }}</span>
                  <span v-else-if="action.result" class="action-result">{{ action.result }}</span>
                  <span v-else class="action-result">{{ action.action_type }} executed</span>
                </div>
                <div v-if="Object.keys(action.action_args || {}).length" class="action-payload">
                  <div v-for="(value, key) in action.action_args" :key="key" class="payload-field">
                    <span class="payload-key">{{ key }}</span>
                    <span class="payload-value">{{ formatPayload(value) }}</span>
                  </div>
                </div>
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Maximum rounds passed from Step2
  minutesPerRound: {
    type: Number,
...
const genericTrackName = computed(() => {
  const tracks = runStatus.value.tracks || {}
  return Object.keys(tracks).find(track => !['twitter', 'reddit'].includes(track)) || 'main'
})

const genericTrack = computed(() => {
  return runStatus.value.tracks?.[genericTrackName.value] || {}
})

const twitterActionsCount = computed(() => {
  if (genericTrack.value.actions_count !== undefined) return genericTrack.value.actions_count
...
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(genericTrack.value.current_round || runStatus.value.twitter_current_round || 0)
})

// Reddit platform simulated elapsed time
const redditElapsedTime = computed(() => {
  return formatElapsedTime(genericTrack.value.current_round || runStatus.value.reddit_current_round || 0)
})
...
const doStartSimulation = async () => {
  ...
  // First reset all state to ensure no impact from previous simulation
  resetAllState()
...
// Track last round for each platform to detect changes and output logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect generic track progress, with legacy platform fallbacks.
      const track = data.tracks?.[genericTrackName.value]
      if (track && track.current_round > prevTwitterRound.value) {
        addLog(`[${genericTrackName.value}] R${track.current_round}/${data.total_rounds} | T:${track.simulated_hours || 0}h | A:${track.actions_count || 0}`)
        prevTwitterRound.value = track.current_round
      } else if (!track && data.twitter_current_round > prevTwitterRound.value) {
        addLog(`[Plaza] R${data.twitter_current_round}/${data.total_rounds} | T:${data.twitter_simulated_hours || 0}h | A:${data.twitter_actions_count}`)
        prevTwitterRound.value = data.twitter_current_round
      }
      
      if (!track && data.reddit_current_round > prevRedditRound.value) {
      ...
      // Check if simulation is completed (via runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: if backend has not updated runner_status yet, but platform has reported completion
...
const checkPlatformsCompleted = (data) => {
  ...
  // Generic tracks are authoritative for domain simulations.
  if (data.tracks && Object.keys(data.tracks).length) {
    return Object.values(data.tracks).every(track => track.status === 'completed')
  }
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Use all_actions (generic tracks included) to get the complete action list.
      const serverActions = res.data.all_actions?.length
        ? res.data.all_actions
        : Object.values(res.data.track_actions || {}).flat()
      ...
      // Do not auto-scroll, let users freely view timeline
...
const getActionTypeLabel = (type) => {
  return type ? String(type).replaceAll('_', ' ') : 'UNKNOWN'
}
...
const handleNextStep = async () => {
  ...
  try {
    ...
    addLog(`✗ Exception starting report generation: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
.action-result { color: #444; }
.action-failure { color: #b42318; font-weight: 600; }
...
.payload-field { display: flex; gap: 8px; }
.payload-key { color: #777; min-width: 90px; }
.payload-value { color: #111; word-break: break-word; }
