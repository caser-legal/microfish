  ...
  <div class="main-view">
    ...
    <header class="app-header">
      <div class="header-left">
        <div class="brand" @click="router.push('/')">MICROFISH</div>
      </div>
...
// Directly get maxRounds from query parameters during initialization, ensuring child components can immediately access the value
...
const handleGoBack = async () => {
  ...
  try {
    // First attempt graceful shutdown of simulation environment
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running, checking if process needs to be stopped
...
const handleNextStep = () => {
  // Step3Simulation component will handle report generation and route navigation directly
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Time config: perround ${minutesPerRound.value} minutes`)
        }
      } catch (configErr) {
        addLog(`Failed to get time config, using default: ${minutesPerRound.value}minutes/round`)
      }
...
const loadGraph = async (graphId) => {
  // When simulating, auto refresh does not show fullscreen loading to avoid flickering
...
const startGraphRefresh = () => {
  ...
  addLog('Start graph real-time refresh (30s)')
...
onMounted(() => {
  ...
  // Record maxRounds config (value already obtained from query params during initialization)
  if (maxRounds.value) {
