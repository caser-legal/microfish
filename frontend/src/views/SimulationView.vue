...
const handleNextStep = (params = {}) => {
  ...
  // Recording simulation rounds configuration
  if (params.maxRounds) {
  ...
  // If custom rounds, pass via query parameter
  if (params.maxRounds) {
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Detected simulation environment running, closing...')
      
      // Attempt graceful shutdown of simulation environment
      try {
        const closeRes = await closeSimulationEnv({ 
          simulation_id: currentSimulationId.value,
          timeout: 10  // 10second timeout
        })
        ...
        if (closeRes.success) {
          ...
          // If graceful shutdown fails, attempt force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Exception closing simulation environment: ${closeErr.message}`)
        // If graceful shutdown exception, attempt force stop
        await forceStopSimulation()
      }
    } else {
    ...
    // Environment check failure doesn't affect subsequent flow
